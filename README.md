...
## U-HPCMS (Node.js + React) — Unified Hospital, Clinic & Pharmacy Management

This repository also includes a **Node.js backend** and **Vite + React frontend** implementing the U-HPCMS workflow (governance, patient flow, triage, consultation, lab, inpatient, pharmacy, clinic, billing in USD/LRD, reporting, audit). Database: **SQLite** (default) or optional PostgreSQL.
...
1. **Backend**
   ...
   cd backend
   npm install
   npm run seed-uhpcms   # one-time: creates demo org, super-admin user, sample data
...
3. **Login (U-HPCMS)**  
   - Email: `super@uhpcms.local`  
   - Password: `admin123`  
   - Or use **Role / Username** for legacy HMS if configured.

### Main flows

- **Governance** (super_admin): Create organizations, enable modules/add-ons, suspend/activate.
- **Org setup**: Departments, wards, pharmacy stores, services (billing codes), users.
- **Patient flow**: Search/register patient → create encounter → triage → consultation (SOAP, order lab, order prescription, Go to Billing, close encounter) → Lab / Pharmacy / Billing.
- **Billing**: USD and Liberian Dollars (LRD); currency switcher in header.
- **Reporting** & **Audit log**: Dashboard stats and audit trail.

Set `VITE_API_URL=http://localhost:3000` in `frontend/.env` if the API runs on a different host.
